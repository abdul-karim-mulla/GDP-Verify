...
class Ion_auth_model extends CI_Model
{
	...
	public function deactivate($id = NULL)
	{
		...
		if (!isset($id))
		{
			...
			return FALSE;
                ...
                elseif($this->ion_auth->logged_in() && $this->user()->row()->user_id == $id)
                {
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, user_id, password, active, last_login')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('user_id', 'desc')
		                  ->get($this->tables['users']);
	...
	**/
	...
	public function login_remembered_user()
	{
		...
		// get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column.', user_id, email, last_login')
